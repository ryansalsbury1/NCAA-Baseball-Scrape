---
title: "NCAA Baseball Scrape Righty Splits"
author: "Ryan Salsbury"
date: "07/13/2018"
output: html_document
---
Scrape player stats vs RHP/RHB for all NCAA division I schools and create new advanced stats. Data consists of season and career stats for both batters and pitchers including ISO, BABIP, wOBA, wRC, K%, BB% and FIP.
...
batting_career_master_vr <- sqldf(paste0("select b.player, '", c ,"' AS Conference , '", s ,"' AS School ,b18.Yr, b.Pos, b.PA, b.AVG, b.OBP, b.SLG, b.HR, b.ISO, b.SB, b.SBA, b.[K%], b.[BB%], b.BABIP, b.wOBA  FROM batting_career b JOIN batting_2018 b18 ON b.Player = b18.Player group by b.Player order by b.PA desc"))
                               
batting_2018_master_vr <-sqldf(paste0("select player,  '", c ,"' AS Conference , '", s ,"' AS School, Yr, Pos, (AB+BB+HBP+SF+SH) AS PA, ROUND(BA,3) AVG, ROUND(OBP,3) OBP, ROUND(SLG,3) SLG, HR, ROUND((SLG-BA), 3) ISO, SB, SBA, ROUND(Kpct,2) AS [K%], ROUND(BBpct,2) AS [BB%], ROUND(BABIP,3) BABIP,  ROUND(wOBA,3) wOBA from batting_2018 where PA > 0 order by PA desc"))
```
...
batting_career_master_vr <- sqldf("Select * from batting_career_master_vr UNION select * from batting_career")
batting_2018_master_vr <- sqldf("Select * from batting_2018_master_vr UNION select * from batting_2018")}

#Export to Excel
#write.csv(x = batting_career_master_vr, file = "NCAA_2018_Career_vr.csv")

#write.csv(x = batting_2018_master_vr, file = "NCAA_2018_Season_vr.csv")
```
...
pitching_career_master_vr <- sqldf(paste0("select p.player, '", c ,"' AS Conference , '", s ,"' AS School ,p18.Yr, p.Pos, p.IP, p.PA, p.Pitches, p.[K%], p.[BB%], p.[K%-BB%], p.AVG, p.OBP, p.SLG, p.wOBA, p.[GO%], p.[FO%], p.ERA, p.FIP FROM pitching_career p JOIN pitching_2018 p18 ON p.Player = p18.Player group by p.Player order by p.PA desc"))
                               
pitching_2018_master_vr <- sqldf(paste0("select p.player, '", c ,"' AS Conference , '", s ,"' AS School ,p.Yr, p.Pos, p.IP, p.PA, p.Pitches, p.[K%], p.[BB%], p.[K%-BB%], p.AVG, p.OBP, p.SLG, p.wOBA, p.[GO%], p.[FO%], p.ERA, p.FIP FROM pitching_2018 p group by p.Player order by p.PA desc"))

```


Get Pitching Stats for all other schools and add to Master Tables
```{r}
...
pitching_career_master_vr <- sqldf("Select * from pitching_career_master_vr UNION select * from pitching_career")

pitching_2018_master_vr <- sqldf("Select * from pitching_2018_master_vr UNION select * from pitching_2018")}

#Export to excel
#write.csv(x = pitching_career_master_vr, file = "NCAA_Pitching_2018_Career_vr.csv")

#write.csv(x = pitching_2018_master_vr, file = "NCAA_Pitching_2018_Season_vr.csv")
